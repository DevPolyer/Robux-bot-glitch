...
module.exports.run = async(client, message, args) =>{
    ...
    const all = users.filter(users => users.guildId === `${message.guild.id}`).map(e => e.coins).reduce((a, b) => a + b)
    const usages = users.filter(users => users.guildId === `${message.guild.id}`).length
